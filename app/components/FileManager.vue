<template>
	<div>
		...
		<UForm :state="folderState" :schema="folderSchema" class="flex flex-col gap-y-4 items-end mt-8" @submit="createFolder">
			...
			<UButton type="submit" size="lg">Create folder</UButton>
		</UForm>
		...
		<div v-if="createdFiles.length" class="mt-6">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div v-for="file in createdFiles" :key="file.name" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
					...
					<UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" class="ml-2" @click.stop="removeFile(file.name)">Remove</UButton>
					<UButton icon="i-lucide-file" color="primary" variant="ghost" size="xs" class="ml-2" @click.stop="openFile(file.name)">Open</UButton>
				</div>
			</div>
		</div>
		...
		<div v-if="createdFolders.length" class="mt-6">
			<h3 class="font-semibold mb-2">Newly Created Folders</h3>
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div v-for="folder in createdFolders" :key="folder.name" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
					<h4 class="font-medium break-all">{{ folder.name }}</h4>
					<p class="text-xs text-gray-500 dark:text-gray-400 break-all">Created just now</p>
					<UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" class="ml-2" @click.stop="removeFolder(folder.name)">Remove</UButton>
					<UButton icon="i-lucide-folder-open" color="primary" variant="ghost" size="xs" class="ml-2" @click.stop="openFolder(folder.name)">Open</UButton>
				</div>
			</div>
		</div>
		...
		<UModal v-model:open="fileContentModalOpen" title="File Contents" @close="fileContentModalOpen = false">
			<template #body>
				<div v-if="fileContentModal">
					<h4 class="font-medium mb-2">{{ fileContentModal.name }}</h4>
		...
		<UModal v-model:open="folderContentModalOpen" title="Folder Contents" @close="folderContentModalOpen = false">
			<template #body>
				<div v-if="folderContentModal">
					<h4 class="font-medium mb-2">{{ folderContentModal.name }}</h4>
					<ul class="list-disc ml-6">
						<li v-for="f in folderContentModal.files" :key="f">{{ f }}</li>
					</ul>
				</div>
			</template>
		</UModal>
...
import { mkdir, BaseDirectory, remove, open, readDir } from '@tauri-apps/plugin-fs';
